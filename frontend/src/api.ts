  ...
  try {
    ...
    if (!isJson) {
      if (response.ok) throw new ApiError(
        'Вместо API получена страница сайта. Проверьте proxy /api и запустите npm.cmd run dev из корня проекта. Используйте свежую копию ветки integrate-frontend-ml-ai.',
        'API_PROXY_MISSING',
      );
      throw new ApiError(
        'Backend не дал JSON-ответ (HTTP ' + response.status + '). Проверьте его запуск на порту 3002 и ошибки в терминале.',
    ...
    throw new ApiError('Нет соединения с сервером. Откройте сайт через http://127.0.0.1:5173 после запуска проекта.', 'NETWORK_ERROR', true);
  } finally {
    clearTimeout(timer);

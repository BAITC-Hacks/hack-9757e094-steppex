...
test('missing Python fails preflight and standalone backend with a safe explanation', () => {
  const env = { ...process.env, PYTHON_BIN: 'steppex-no-such-python-executable', OPENAI_API_KEY: 'do-not-print-this-test-value' };
  ...
  for (const script of ['scripts/check-env.js', 'src/server.js']) {
    ...
    assert.doesNotMatch(result.stderr + result.stdout, /do-not-print-this-test-value|SteppeX backend:/);
  }
});

...
test("official baseline and district table match the specified formula", () => {
...
test("official example: budget 95, no critical values, score 56.54307", () => {
  ...
  const nura = r.districts.find((x) => x.id === "nura");
  assert.equal(nura.after.S1, 48);
  assert.equal(nura.after.S2, 43.75);
  assert.equal(nura.after.B1, 67.5); // Includes fixed +2 synergy, not scaled.
...
test("critical threshold is strictly below 40 and negative effects are retained", () => {
  ...
  const almaty = r.districts.find((x) => x.id === "almaty");
  assert.equal(almaty.after.T1, 38.25);
  assert.ok(
    r.criticalIndicators.some(
      (x) => x.districtId === "almaty" && x.indicatorId === "T1",
    ),
  );
  assert.ok(
...
test("effects are clipped after summation, with order-independent results", () => {
  ...
  assert.equal(r.districts.find((x) => x.id === "nura").after.T1, 100);

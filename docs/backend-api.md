...
## Продолжение диалога (локальная интеграция)

Успешный `POST /api/analyze` дополнительно возвращает:
```json
{"conversation":{"id":"UUID","revision":0,"priority":null,"turnsRemaining":5,"expiresAt":0}}
```
`expiresAt` — фактическое время истечения в миллисекундах Unix (в примере 0 — заполнитель).

### POST /api/conversations/:id/messages

```json
{"message":"Как сохранить резерв?","priority":"reserve","revision":0,"requestId":"новый UUID"}
```
Приоритет необязателен: `quality | equity | reserve`. Сообщение от 1 до 1500 символов. Передаются только текст, приоритет, версия и UUID запроса; поля истории, решений, бюджета и результатов запрещены.

Ответ: `{kind, message, conversation, report?}`. `kind=clarification` — бесплатное уточнение цели для короткого «Да, помоги», явно не результат модели. `kind=analysis` содержит новый проверенный `report` той же формы, что исходный анализ, с usage за этот ответ. Каждый платный ответ заново вызывает compare_strategies/inspect_risks. Лимит частоты общий с /api/analyze.

### POST /api/conversations/:id/apply

```json
{"strategyId":"reserve","revision":1,"requestId":"новый UUID"}
```
Сервер берёт только последний сохранённый вариант и повторно проверяет его симулятором. Ответ: `{kind:"applied",message,conversation,simulation}`. Вызов бесплатен в смысле API модели. История сохраняется, текущий набор заменяется, предыдущие варианты становятся недоступны до нового сравнения.

### Ограничения и ошибки

- `revision` берётся из последнего ответа. Каждый успешный шаг увеличивает её.
- Повтор того же успешного запроса с тем же UUID и телом возвращает сохранённый ответ. UUID нельзя использовать для другого тела.
- Ошибки: `CONVERSATION_EXPIRED` (410), `CONVERSATION_CHANGED`, `CONVERSATION_BUSY`, `STRATEGY_UNAVAILABLE`, `REQUEST_ID_REUSED`, `CONVERSATION_TURN_LIMIT` (409); `AI_RATE_LIMIT` (429).
- Неудачный ответ не добавляется в историю. Не повторяйте платный вызов автоматически.
- До шести платных ответов на диалог (включая первый), до тридцати последующих операций; до пятидесяти диалогов в памяти, TTL тридцать минут бездействия. Перезапуск удаляет контекст.
- Идентификатор — непрогнозируемый UUID, но это не замена авторизации; сохраняется ограничение на локальную демонстрацию. Модель получает только последние восемь ограниченных сообщений.
- Фронтенд сбрасывает диалог при ручном изменении мер/перезагрузке. Применение через /apply, наоборот, сохраняет диалог и обновляет его сценарий.

...
Локальный адрес: `http://127.0.0.1:3001`. Все POST-запросы содержат `Content-Type: application/json`. Поля `budget`, `cost`, `score`, пользовательские эффекты и неизвестные поля запрещены. Числовой источник истины — сервер.
